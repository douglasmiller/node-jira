...
var JiraApi = exports.JiraApi = function(protocol, host, port, username, password, apiVersion, verbose) {
    ...
    // later, easier
    this.makeUri = function(pathname, altBase, altVersion) {
        var basePath = 'rest/api/';
        if (altBase != null) {
            basePath = altBase;
        }
        var version = altVersion != null ? altVersion : this.apiVersion;
        ...
        var uri = url.format({
            ...
            port: this.port,
            pathname: basePath + version + pathname
        });
        return uri;
    };
};
...
(function() {
    ...
    // ## Add an issue(s) to an Epic ##
    // ### Takes ###
    //
    // *  epicKey: the key of the existing issue
    // *  epicLink: an epiclink object
    // *  callback: for when it's done
    //
    // ### Returns ###
    // 
    ...
    //
    // **does this callback if there's success?**
    // [Closest Doc](https://confluence.atlassian.com/display/GHKB/Set+the+%27Epic+Link%27+via+REST+call)
    /**
     * Adds a given issue to a project's current sprint
     *
     * @param epicKey
     ...
     * @param callback
     */
    this.addIssueToEpic = function(epicKey, epicLink, callback) {
        var options = {
